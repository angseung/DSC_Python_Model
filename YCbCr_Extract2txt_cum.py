...
This script can detect subsampling type automatically.
"""
...
for img_file in input_file_list:
    ...
    if "422" in img_file:
        print("%s is 422 image" % img_file)
        ### For 422 Mode
        frame_422 = np.fromfile(file_path + img_file, dtype = 'uint8', offset = 0) # packed format (UYVY)
        ...
        np.savetxt('HEX_422/%s_422_Y.txt' % (img_file[:-12]), reshape_422_Y, fmt='%x', delimiter='\n')
        np.savetxt('HEX_422/%s_422_Cb.txt' % (img_file[:-12]), reshape_422_Cb, fmt='%x', delimiter='\n')
        np.savetxt('HEX_422/%s_422_Cr.txt' % (img_file[:-12]), reshape_422_Cr, fmt='%x', delimiter='\n')

        # is_Valid_Cb_422 = reshape_422_Cb - valid_Cb_422
        # is_Valid_Cr_422 = reshape_422_Cr - valid_Cr_422

    if "420" in img_file:
        print("%s is 420 image" % img_file)
        ## For 420 Mode
        frame_420 = np.fromfile(file_path + img_file, dtype = 'uint8', offset = 0) # packed format (YUV)
        ...
        np.savetxt('HEX_420/%s_420_Y.txt' % (img_file[:-12]), raw_420_Y, fmt='%x', delimiter='\n')
        np.savetxt('HEX_420/%s_420_Cb.txt' % (img_file[:-12]), mod_420_Cb, fmt='%x', delimiter='\n')
        np.savetxt('HEX_420/%s_420_Cr.txt' % (img_file[:-12]), mod_420_Cr, fmt='%x', delimiter='\n')
