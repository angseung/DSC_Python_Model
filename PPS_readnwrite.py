...
with open("sample_01.dsc", "rb") as f:
    ...
    PPS["RESERVED0"] = int.from_bytes(f.read(1), byteorder = 'big', signed = 0)  # RESERVED 8bits
    ...
    PPS["RESERVED1"] = 0x0
    PPS["blockpred_enable"] = (tmp >> 13) & 0b1
    ...
    PPS["RESERVED13"] = int.from_bytes(f.read(34), byteorder = 'big', signed = 0)

if PRINT_PPS_OPT :
    for key, val in PPS.items():
        print("PPS : %s, Value : " %key, val)

# PPS_size = [4,4,8,8,4,4,2,1,1,1,1,10,16,16,16,16,16,6,10,16,10,6,16,4,12,11,5,16,16,16,16,3,5,3,5,16,4,4,3,5,3,5,4,4,
#             np.array([8,8,8,8,8,8,8,8,8,8,8,8,8,8]),
#             np.array([[5,5,6],
#              [5,5,6],
#              [5,5,6],
#              [5,5,6],
...
with open("pps_write_test.txt", "wb") as f:
    ...
    tmp = PPS["RESERVED0"].to_bytes(1, 'big')
    f.write(tmp)

    tmp = (PPS["bits_per_component"] * (2 ** 4) + PPS["linebuf_depth"]).to_bytes(1, 'big')
    f.write(tmp)

    tmp = (PPS["blockpred_enable"] * (2 ** 13) + PPS["convert_rgb"] * (2 ** 12) + PPS["simple_422"] * (2 ** 11) + PPS["vbr_enable"] * (2 ** 10) + PPS["bits_per_pixel"]).to_bytes(2, 'big')
    f.write(tmp)
    ...
    tmp = PPS['RESERVED13'].to_bytes(34, 'big')
    f.write(tmp)